...
class DescriptionSchema(BaseModel):
    """MAM description/template settings."""

    show_signature: bool = Field(
        default=True, description="Show signature/branding in MAM descriptions"
    )
    signature_template: str = Field(
        default="signature.j2",
        description="Filename of signature template in templates/ dir",
    )
...
class MamSchema(BaseModel):
    ...
    description: DescriptionSchema = Field(default_factory=DescriptionSchema)
