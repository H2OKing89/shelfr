...
async def check_docker_available() -> bool:
    ...
    try:
        ...
        if proc.returncode != 0:
            console.print(
                f"[red]✗ Docker image test failed:[/red] {stderr.decode(errors='replace')[:200]}"
            )
            return False
...
async def probe_file(filepath: Path) -> tuple[Path, dict | None, str | None]:
    ...
    try:
        ...
        metadata = {
            ...
            .isoformat()
            .replace("+00:00", "Z"),
            "probe_timestamp_utc": datetime.now(UTC).isoformat().replace("+00:00", "Z"),
...
async def process_all_files(audio_files: list[Path]) -> tuple[int, int, list[tuple[str, str]]]:
    ...
    - Bounded queues with backpressure to prevent memory exhaustion on 100k+ files
    """
    # Bounded queues: prevent unbounded memory growth for large file counts
    work_queue: asyncio.Queue = asyncio.Queue(maxsize=MAX_WORKERS * 10)
    results_queue: asyncio.Queue = asyncio.Queue(maxsize=MAX_WORKERS * 2)

    async def producer():
        """Fill work queue with backpressure."""
    ...
    with Progress(
        ...
        # Spawn producer, workers, and result writer with coordinated lifecycle
...
def display_summary(success: int, failed: int, failures: list[tuple[str, str]], total: int) -> None:
    ...
    if failures:
        ...
        for filename, error in failures[:10]:
            fail_table.add_row(
                filename[:55] + "..." if len(filename) > 55 else filename, error[:50]
            )
